    ...
    public SkillResult tick(SkillContext c) {
        ...
        for (BlockPos floor : BlockPos.betweenClosed(new BlockPos(pen.originX - 1, pen.originY - 1, pen.originZ - 1),
            ...
            for (int dy = 3; dy >= 1; dy--) {
                ...
                if (!Pens.natural(c.level.getBlockState(at))) continue;
            ...
            if (!c.level.getBlockState(at).canBeReplaced() || !c.level.getFluidState(at).isEmpty()
            ...
            if (c.level.setBlock(at, Blocks.DIRT.defaultBlockState(), 3)) c.citizen.getInventory().extract("minecraft:dirt", 1);
